...
We would like to approximate the posterior by a tractable distribution $q(\boldsymbol{\theta})$. In this clutter problem, we choose spherical Gaussian: 
$$
q(\boldsymbol{\theta})=\mathcal{N}\left(\boldsymbol{\theta} \vert \mathbf{m}_{\theta}, v_{\theta}\mathbf{I}\right), \label{eq:spherical-gaussian}
$$
and the learnable parameters are $\boldsymbol{\theta}=\{\mathbf{m}_\theta, v_\theta\}$.

#### Calculate the approximate posterior

Suppose that given datapoints $\mathcal{D}_{i-1}=\{\mathbf{x}_1,\mathbf{x}_2,\cdots,\mathbf{x}_{i-1}\}$, our current estimate of the posterior is $q(\boldsymbol{\theta}\vert \mathcal{D}_{i-1})$ given by Eq. ($\ref{eq:spherical-gaussian}$). Then after observing a new datapoint $\mathbf{x}_{i}$, we can update the posterior using Bayes' rule: 
$$
p(\boldsymbol{\theta}\vert \mathcal{D}_{i}) \propto p(\mathbf{x}_{i} \vert \boldsymbol{\theta}) q(\boldsymbol{\theta}\vert \mathcal{D}_{i-1}),
$$
...
After obtaining the new posterior in Eq. ($\ref{eq:update-new-posterior}$), the final step is to update our parameters $\boldsymbol{\theta}=\{\mathbf{m}_\theta, v_\theta\}$ so that the new $q(\boldsymbol{\theta})$ is as close as possible to $\hat{p}(\boldsymbol{\theta})$. This is done by *moment matching*, which we will explain later. In this way, we can sequentially update our estimate of the posterior, one after another.

### Details in ADF

#### Moment matching

Here we explain what *moment matching* is. In order to update our parameters $\mathbf{m}_\theta$ and $v_\theta \mathbf{I}$, we choose to minimize the forward KL divergence (not the reverse KL divergence we use in Variational Inference): 
$$
\begin{align}
    \label{eq:KL}
...
\end{align}
$$
Taking derivatives with respect to $\mathbf{m}_{\theta}$ and $v_\theta$ gives: 
$$
\begin{align}
    \nabla_{\mathbf{m}_\theta} D_{\text{KL}} \left[\hat{p}(\boldsymbol{\theta}) \parallel q(\boldsymbol{\theta}) \right] &= - \int \hat{p}(\boldsymbol{\theta}) \frac{\partial \ln q(\boldsymbol{\theta})}{\partial \mathbf{m}_\theta} \text{d}\boldsymbol{\theta}\nonumber \\
    &= - \int \hat{p}(\boldsymbol{\theta}) \frac{\boldsymbol{\theta}- \mathbf{m}_\theta}{v_\theta} \text{d}\boldsymbol{\theta}\\
    \nabla_{v_\theta} D_{\text{KL}} \left[\hat{p}(\boldsymbol{\theta}) \parallel q(\boldsymbol{\theta}) \right] &= - \int \hat{p}(\boldsymbol{\theta}) \frac{\partial \ln q(\boldsymbol{\theta})}{\partial v_\theta} \text{d}\boldsymbol{\theta}\nonumber \\
    &= - \int \hat{p}(\boldsymbol{\theta}) \left[-\frac{d}{2v_\theta} + \frac{1}{2v_\theta^2} \left( \boldsymbol{\theta}^\top \boldsymbol{\theta}-2 \boldsymbol{\theta}^\top \mathbf{m}_\theta + \left\lVert\mathbf{m}_\theta\right\rVert^2\right) \right] \text{d}\boldsymbol{\theta}
\end{align}
$$
By setting derivatives to zero and rearranging equations, we have 
$$
\begin{align}
    \label{eq:moment-matching-1}
    \mathbf{m}_\theta &= \int \hat{p}(\boldsymbol{\theta}) \boldsymbol{\theta}\text{d}\boldsymbol{\theta}= \mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}\right] \\
    \label{eq:moment-matching-2}
    v_\theta d + \mathbf{m}_\theta^\top \mathbf{m}&= \int \hat{p}(\boldsymbol{\theta}) \boldsymbol{\theta}^\top \boldsymbol{\theta}\text{d}\boldsymbol{\theta}= \mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}^\top \boldsymbol{\theta}\right]
\end{align}
$$

#### How to compute the expectations

The next question is: how to compute the expectations on the r.h.s of Eq. ($\ref{eq:moment-matching-1}, \ref{eq:moment-matching-2}$)? We resort to the following property: for arbitrary function $t(\boldsymbol{\theta})$ and spherical Gaussian $q(\boldsymbol{\theta})=\mathcal{N}(\mathbf{m}_{\theta},v_{\theta}\mathbf{I})$, we have 
$$
\begin{align}
...
For the clutter problem, there exits analytical solution. To recap, our current estimate (after observing $\{\mathbf{x}_1, \cdots, \mathbf{x}_{i-1}\}$) of the posterior is given by Eq. ($\ref{eq:spherical-gaussian}$). Then after observing a new datapoint $x_i$, we calculate the expectations of $\hat{p}(\boldsymbol{\theta})$ and then set $\mathbf{m}^{\text{new}}_{\boldsymbol{\theta}}$ equal to $\mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}\right]$ and $d v^{\text{new}}_{\theta}$ equal to $\mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}^\top \boldsymbol{\theta}\right] - \left(\mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}\right]\right)^\top \mathbb{E}_{\hat{p}}\left[\boldsymbol{\theta}\right]$. First, we calculate the normalizing factor $Z_i$: 
$$
\begin{align}
...
Suppose that given all datapoints $\mathcal{D}_N=\{\mathbf{x}_1,\mathbf{x}_2,\cdots,\mathbf{x}_N\}$, our estimate of the posterior is $q(\boldsymbol{\theta})$ given by Eq. ($\ref{eq:product-of-factors}$). Every time we randomly remove $\mathbf{x}_i$. After $\mathbf{x}_i$ being removed, our current estimate of the "cavity" posterior $q^{\setminus i}(\boldsymbol{\theta})$ is given by
$$
\begin{align}
...
$$
\begin{align}
q^{\text{new}}(\boldsymbol{\theta})=\frac{t_i(\boldsymbol{\theta})q^{\setminus i}(\boldsymbol{\theta})}{\int t_i(\boldsymbol{\theta})q^{\setminus i}(\boldsymbol{\theta}) \text{d}\boldsymbol{\theta}}=\frac{t_i(\boldsymbol{\theta})q^{\setminus i}(\boldsymbol{\theta})}{Z_i}.
\end{align}
$$

#### Update parameters

Since now we have updated our new posterior, we can refine $\tilde{t}_i(\boldsymbol{\theta})$ according to 
$$
...
#### Calculate the cavity distribution

We use the following result: the division of two spherical Gaussian 
$$
\begin{align}
